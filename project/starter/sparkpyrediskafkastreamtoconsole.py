...
from sparkpykafkajoin import redisMessageSchema

# TO-DO: create a StructType for the Kafka redis-server topic which has all changes made to Redis - before Spark 3.0.0, schema inference is
# not automatic
...
# TO-DO: create a StructType for the Kafka stedi-events topic which has the Customer Risk JSON that comes from Redis- before Spark 3.0.0, 
...
spark = SparkSession.builder.appName("customer_record").getOrCreate()
...
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark 
# stream
redisServerStreamDF = spark \
    .readStream \
    .format("kafka") \
...
redisServerStreamDF = redisServerStreamDF.selectExpr("cast(value as string) value")
...
redisServerStreamDF.withColumn("value", from_json("value", redisMessageSchema)) \
    .select(*columnNames) \
...
# and create a column called encodedCustomerthe reason we do it this way is that the syntax available select against a view is different than
# a dataframe, and it makes it easy to select the nth element of an array in a sql column
...
# TO-DO: JSON parsing will set non-existent fields to null, so let's select just the fields we want, where they are not null as a new 
# dataframe called emailAndBirthDayStreamingDF
...
emailAndBirthYearStreamingDF = emailAndBirthDayStreamingDF.select('email', split(emailAndBirthDayStreamingDF.birthDay, "-")\
